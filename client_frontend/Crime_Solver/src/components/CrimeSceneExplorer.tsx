...
import { 
  ...
  Sparkles, CheckCircle2, Clock, Eye, EyeOff, Tag, ListChecks, MapPin, UserMinus
} from 'lucide-react';
...
export const CrimeSceneExplorer: React.FC<CrimeSceneExplorerProps> = ({
  ...
  const getHotspotIcon = (iconType: string) => {
    switch (iconType) {
      case 'file': return <FileText className="w-5 h-5 text-orange-300" />;
      case 'key': return <Key className="w-5 h-5 text-orange-300" />;
      case 'flask-conical': return <FlaskConical className="w-5 h-5 text-red-300" />;
      case 'camera': return <Camera className="w-5 h-5 text-orange-300" />;
      default: return <Search className="w-5 h-5 text-orange-300" />;
    }
  };
  ...
  return (
    <div className="relative min-h-screen flex flex-col z-10 select-none bg-[#090c12] text-slate-100">
      ...
      <header className="sticky top-0 z-30 bg-[#0f1420]/95 border-b border-orange-500/30 px-4 sm:px-6 py-3 flex flex-wrap items-center justify-between gap-3 shadow-2xl backdrop-blur-md">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="px-3 py-1.5 rounded-lg bg-slate-900 border border-slate-700 hover:border-orange-400 text-slate-200 hover:text-white transition-colors cursor-pointer flex items-center gap-1.5 font-mono text-xs font-semibold"
          >
          ...
          <div>
            <div className="flex items-center gap-2">
              <span className="font-creepster font-bold text-sm text-white tracking-wide">
                {caseData.caseNumber}
              </span>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            {isNightVisionOn ? <Eye className="w-4 h-4 text-red-400" /> : <EyeOff className="w-4 h-4 text-orange-400" />}
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg bg-red-950/80 border border-red-700 hover:border-red-400 text-red-300 font-mono text-xs font-bold transition-colors cursor-pointer animate-pulse"
          >
            <UserMinus className="w-4 h-4 text-red-400" />
            <span className="hidden lg:inline">INTERROGATE</span>
          </button>
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg bg-slate-900 border border-slate-700 hover:border-orange-400 text-orange-300 font-mono text-xs font-bold transition-colors cursor-pointer"
          >
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-4 py-1.5 rounded-lg bg-gradient-to-r from-red-600 to-orange-600 hover:from-red-500 hover:to-orange-500 text-white font-creepster font-bold text-xs tracking-wider shadow-lg hover:shadow-red-500/25 transition-all cursor-pointer"
          >
      ...
      <div className="bg-[#0b0e17] border-b border-slate-800 px-6 py-2 flex flex-wrap items-center justify-between text-xs font-sans">
        <div className="flex items-center gap-6 text-slate-300">
          <div className="flex items-center gap-2 font-medium">
            <ListChecks className="w-4 h-4 text-orange-400 shrink-0" />
            <span className="font-bold text-slate-100">OBJECTIVES:</span>
          </div>

          <div className="flex items-center gap-1.5">
            <span className={`w-4 h-4 rounded-full flex items-center justify-center text-[10px] font-bold ${
              discoveredClues.length > 0 ? 'bg-rose-500 text-slate-950' : 'bg-slate-800 text-slate-400'
            }`}>
              {discoveredClues.length > 0 ? '✓' : '1'}
            </span>
            <span className={discoveredClues.length >= caseData.clues.length ? 'text-rose-400 font-semibold' : 'text-slate-300'}>
          ...
          <div className="flex items-center gap-1.5">
            <span className={`w-4 h-4 rounded-full flex items-center justify-center text-[10px] font-bold ${
              discoveredClues.length >= 2 ? 'bg-rose-500 text-slate-950' : 'bg-slate-800 text-slate-400'
            }`}>
        ...
        <div className="hidden lg:flex items-center gap-2 text-orange-400 font-mono text-[11px]">
      ...
      {scanMessage && (
        <div className="fixed top-24 left-1/2 -translate-x-1/2 z-50 bg-orange-500 text-slate-950 font-bold px-6 py-2.5 rounded-full shadow-2xl flex items-center gap-2 text-xs font-mono border-2 border-slate-950 animate-bounce">
      ...
      <main className="flex-1 relative flex items-center justify-center p-4 overflow-hidden">
        ...
        >
          ...
          {!isNightVisionOn && isHoveringScene && (
            <div
              className="absolute w-12 h-12 rounded-full border-2 border-orange-400/40 pointer-events-none -translate-x-1/2 -translate-y-1/2 transition-transform duration-75"
              style={{ left: `${mousePos.x}%`, top: `${mousePos.y}%` }}
            >
              <div className="absolute inset-0 rounded-full border border-orange-300 animate-ping opacity-30" />
            </div>
          )}

          {/* Room Title Plate */}
          <div className="absolute top-6 left-6 z-20 pointer-events-none text-left">
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded bg-[#0b0e14]/90 border border-slate-700 text-orange-400 font-mono text-xs font-bold mb-1 shadow-lg">
              ...
              >
                {isDiscovered ? (
                  ...
                  <div className="flex flex-col items-center">
                    <div className="bg-orange-400 text-slate-950 font-mono text-[10px] font-black px-2 py-0.5 rounded shadow-lg flex items-center gap-1 border border-slate-950 animate-in zoom-in-75">
                      <Tag className="w-3 h-3 text-slate-950" />
                      <span>{tagNumber} LOGGED</span>
                    </div>
                    <div className="w-9 h-9 mt-1 rounded-full bg-rose-500 text-slate-950 flex items-center justify-center font-bold shadow-lg border-2 border-slate-950 group-hover/hotspot:scale-110 transition-transform">
                  ...
                  <div className="flex flex-col items-center">
                    <div className="w-12 h-12 rounded-full bg-orange-950/90 border-2 border-orange-400 text-orange-300 flex items-center justify-center radar-marker shadow-2xl group-hover/hotspot:scale-115 transition-transform duration-200">
                      {getHotspotIcon(hotspot.iconType)}
                    </div>
                    <span className="mt-1 font-mono text-[10px] font-bold text-orange-300 bg-black/80 px-2 py-0.5 rounded border border-orange-500/60 shadow-md whitespace-nowrap">
                      🔎 CLICK TO EXAMINE
                    </span>
                  </div>
                )}
                ...
                <div className="absolute bottom-14 left-1/2 -translate-x-1/2 w-52 bg-[#0d121c] border border-orange-400/80 p-3 rounded-xl text-left pointer-events-none opacity-0 group-hover/hotspot:opacity-100 transition-opacity duration-200 shadow-2xl z-30">
          ...
          <div className="absolute bottom-4 left-6 z-20 text-[11px] font-sans text-slate-300 bg-black/70 px-3 py-1.5 rounded-lg border border-slate-800 backdrop-blur-sm flex items-center gap-2 pointer-events-none">
            <Flashlight className="w-3.5 h-3.5 text-orange-400" />
