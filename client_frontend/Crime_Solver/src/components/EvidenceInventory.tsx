...
export const EvidenceInventory: React.FC<EvidenceInventoryProps> = ({
  ...
  const getClueIcon = (type: string) => {
    switch (type) {
      ...
      case 'PHOTO': return <Camera className="w-4 h-4 text-amber-400" />;
      case 'OBJECT': return <Key className="w-4 h-4 text-amber-400" />;
      case 'FORENSIC': return <FlaskConical className="w-4 h-4 text-cyan-400" />;
      default: return <FileText className="w-4 h-4 text-amber-400" />;
    }
  };

  return (
    <div className="fixed inset-y-0 right-0 w-full sm:w-[480px] z-50 bg-[#0c1018]/95 border-l-2 border-amber-500/40 shadow-2xl flex flex-col backdrop-blur-xl animate-in slide-in-from-right duration-300 select-none">
          ...
          discoveredClues.length === 0 ? (
            ...
            discoveredClues.map((clue, idx) => (
              <div
                ...
                className="bg-[#131926] hover:bg-[#182133] border border-slate-800 hover:border-amber-400/80 p-4 rounded-xl transition-all cursor-pointer group text-left shadow-lg"
              >
                <div className="flex items-center justify-between mb-1.5">
                  <div className="flex items-center gap-2">
                    <span className="bg-amber-400 text-slate-950 font-mono text-[10px] font-black px-1.5 py-0.5 rounded">
                      #0{idx + 1}
                    </span>
                  ...
                  <Eye className="w-4 h-4 text-slate-500 group-hover:text-amber-300 transition-colors" />
                </div>
                <h4 className="text-sm font-sans font-bold text-white group-hover:text-amber-200">
                  {clue.title}
                </h4>
          ...
          allSuspects.map((suspect) => (
            ...
            >
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-amber-950/80 border border-amber-500 flex items-center justify-center text-amber-300 font-mono font-black text-sm">
                  {suspect.name.charAt(0)}
                </div>
                <div>
                  <h4 className="text-sm font-sans font-bold text-white">{suspect.name}</h4>
                  <span className="text-xs font-mono text-amber-400">{suspect.role} (Age {suspect.age})</span>
                </div>
              </div>
